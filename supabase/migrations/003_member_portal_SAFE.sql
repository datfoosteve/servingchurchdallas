-- Member Portal Schema (SAFE VERSION - Can be run multiple times)
-- This version uses DROP IF EXISTS to prevent errors
...
DROP POLICY IF EXISTS "Members can view all profiles" ON public.members;
...
CREATE POLICY "Members can view all announcements" ON public.announcements
  FOR SELECT TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Pastor/Admin can manage announcements" ON public.announcements;
...
DROP POLICY IF EXISTS "Members can view own reads" ON public.announcement_reads;
CREATE POLICY "Members can view own reads" ON public.announcement_reads
  FOR SELECT TO authenticated
  USING (member_id = auth.uid());

DROP POLICY IF EXISTS "Members can mark as read" ON public.announcement_reads;
CREATE POLICY "Members can mark as read" ON public.announcement_reads
  FOR INSERT TO authenticated
  WITH CHECK (member_id = auth.uid());

-- Polls policies
DROP POLICY IF EXISTS "Members can view active polls" ON public.polls;
CREATE POLICY "Members can view active polls" ON public.polls
  FOR SELECT TO authenticated
  USING (is_active = TRUE);

DROP POLICY IF EXISTS "Pastor/Admin can manage polls" ON public.polls;
...
DROP POLICY IF EXISTS "Pastor/Admin can manage poll options" ON public.poll_options;
...
-- Poll votes policies
DROP POLICY IF EXISTS "Members can view own votes" ON public.poll_votes;
CREATE POLICY "Members can view own votes" ON public.poll_votes
  FOR SELECT TO authenticated
  USING (member_id = auth.uid());
...
-- Function to auto-create member profile when user signs up
...
-- Trigger to auto-create member profile
